/*****************************************************************************\
 *  aucsched_wrapper.c - plugin for SLURM backfill scheduler.
...
const char		plugin_type[]	= "sched/aucsched";
const uint32_t		plugin_version	= 100;

/* A plugin-global errno. */
static int plugin_errno = SLURM_SUCCESS;

static pthread_t aucsched_thread = 0;
...
int init( void )
{
	...
	if (pthread_create( &aucsched_thread, &attr, aucsched_agent, NULL))
...
void fini( void )
{
	...
	if ( aucsched_thread ) {
		...
		stop_aucsched_agent();
		pthread_join(aucsched_thread, NULL);
		aucsched_thread = 0;
	}
	pthread_mutex_unlock( &thread_flag_mutex );
}
