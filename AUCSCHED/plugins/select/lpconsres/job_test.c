			 ...
			 uint16_t *cpu_cnt)
{
	...
	ec = _eval_nodes(job_ptr, node_map, min_nodes, max_nodes,
        ...
        debug3("eval nodes returned EC %d",ec);
	if (ec == SLURM_SUCCESS) {
				...
				uint16_t cr_type, bool test_only)
{
	...
	*/
	...
	rc = _choose_nodes(job_ptr, node_map, min_nodes, max_nodes, req_nodes,
			   cr_node_cnt, cpu_cnt);
        debug3("choose nodes returned %d",rc);
	/*messagingdebug
	bit_fmt(str,sizeof(str),node_map);
	...
	*/
	...
	if (rc == SLURM_SUCCESS) {
		/*messagingdebug*/
		debug3("_select_nodes after _choose_nodes slurm_success");
			...
			struct node_use_record *node_usage)
{
	...
	cpu_count = _select_nodes(job_ptr, min_nodes, max_nodes, req_nodes,
        ...
        //debug3("select nodes returned %d",cpu_count);
	if (cpu_count == NULL) {
		/* job cannot fit */
