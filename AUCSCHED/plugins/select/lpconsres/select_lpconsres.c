		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	...
	rc = cr_job_test(job_ptr, bitmap, min_nodes, max_nodes, req_nodes,
			 ...
			 select_node_usage);
	debug3("cr_job_test returned %d",rc);
	return rc;
}
			     ...
			     List *preemptee_job_list)
{
	...
	debug3("select_p_job_test returned %d",rc);
	return rc;
}
	...
	for (n=0; n < select_node_cnt; n++) {
		...
		*/
		...
		if (select_node_usage[n].gres_list) {
			...
			debug3("node %d, select_node_usage[%d].gres_list exists",n,n);
		}
		else {
			gres_list = node_ptr->gres_list;
                        debug3("node %d, select_node_usage[%d].gres_list does not exist",n,n);
		}
		if (gres_list) {
			remaining_gpu = gres_plugin_node_state_log(gres_list, node_ptr->name);
	                debug3("node %d, gres_list exists, remaining_gpu %d",n,remaining_gpu);
		} else {
	                debug3("node %d, gres_list does not exist, remaining_gpu %d",n,remaining_gpu);
		...
		if ( ( (IS_NODE_ALLOCATED(node_ptr)) || (IS_NODE_IDLE(node_ptr)) )
