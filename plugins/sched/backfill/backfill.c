...
serenkod
*/
void _print_nodes_inbackfill()
{
/*	struct node_schedinfo *ninfo;
	int select_node_cnt;
	info("in printnodes");
	ninfo = _return_nodes(&select_node_cnt);
	info("returned from returnnodes");
	int i;
	for (i=0; i<select_node_cnt; i++) {
		info("in loop for node %d in printnodes",i);
		info("info in printnodes: cpu (rem): %u, mem (rem): %u",ninfo[i].rem_cpus,ninfo[i].rem_memory);
	}
*/
/*	select_nodeinfo_t *nodeinfo_ptr = NULL;

	nodeinfo_ptr = select_g_select_nodeinfo_alloc();
*/
	/*int x = select_g_select_nodeinfo_alloc();*/
	char* dummy;
	info("currently in _print_nodes_inbackfill();");
	struct node_schedinfo* nodeinfo_ptr = NULL;
	info("testing select %d",select_g_state_save(dummy));
}
...
/*	while (0) {*/
		...
		if ((int)((job_ptr->job_id) % 2) == 1) {
			debug("backfilling job id %u is even, skipping!",job_ptr->job_id);
			continue;
		}
