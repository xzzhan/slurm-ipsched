...
extern int gres_job_gpu_count(List job_gres_list) {
	...
	debug("  loop in gres_job_gpu_count");
		for (i=0; i<gres_context_cnt; i++) {
			if (strcmp(gres_context[i].gres_name,"gpu") == 0) {
				job_gres_ptr2 = (gres_job_state_t *)job_gres_ptr->gres_data;
				debug("gpu found, %d.",job_gres_ptr2->gres_cnt_alloc);
...
{
	debug2("in grespluginnodestatelog");
	info("gresp lugin hedheheuhueahue");
	int i, k = 0, remaining;
	ListIterator gres_iter;
	gres_state_t *gres_ptr;
