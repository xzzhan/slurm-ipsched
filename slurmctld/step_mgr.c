					       ...
					       uint16_t plane_size)
{
	...
	debug("first bit: %d, last bit: %d",first_bit, last_bit);
	bit_fmt(str, sizeof(str), job_ptr->node_bitmap);
	debug("node bitmap: %s",str);
	for (i = first_bit; i <= last_bit; i++) {
		if (!bit_test(job_ptr->node_bitmap, i))
			continue;
		job_node_offset++;
		if (bit_test(step_ptr->step_node_bitmap, i)) {
			...
			if (step_ptr->exclusive) {
				...
				debug("exclusive. usable_cpus: %d, pos: %d, job_resrcs_ptr->cpus[pos]: %d, job_resrcs_ptr->cpus_used[pos]: %d",usable_cpus,pos, job_resrcs_ptr->cpus[pos], job_resrcs_ptr->cpus_used[pos]);
			} else {
				usable_cpus = job_resrcs_ptr->cpus[pos];
				debug("else. usable_cpus: %d, pos: %d, job_resrcs_ptr->cpus[pos]: %d, job_resrcs_ptr->cpus_used[pos]: %d",usable_cpus,pos, job_resrcs_ptr->cpus[pos], job_resrcs_ptr->cpus_used[pos]);
			}
			if (step_ptr->mem_per_cpu && _is_mem_resv()) {
				usable_mem =
			...
			gres_cpus = gres_plugin_step_test(step_ptr->gres_list,
							  ...
							  job_ptr->job_id,
							  step_ptr->step_id);
			debug("usable cpus: %d, gres_cpus: %d",usable_cpus, gres_cpus);
